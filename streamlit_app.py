...
st.set_page_config(
    page_title="PEI 360º Visual Map",
...
# ==============================================================================
# 6. INTELIGÊNCIA ARTIFICIAL (V92 - PROMPT COM BLINDAGEM)
# ==============================================================================
...
# --- FUNÇÃO DALL-E 3 ---
def gerar_imagem_dalle(api_key, dados_aluno, texto_estrategias):
    if not api_key: return None, "Configure a API Key."
    if not texto_estrategias: return None, "Texto não encontrado."
    try:
        client = OpenAI(api_key=api_key)
        hf = dados_aluno['hiperfoco'] if dados_aluno['hiperfoco'] else "aprendizado criativo"
        
        prompt_dalle = f"""
        A creative infographic illustration of a 'Student Power Map' pinned on a corkboard.
        Title: "MEU MAPA". Theme: {hf}.
        Visuals: Drawings representing these strategies:
        {texto_estrategias[:500]}
        Style: Pixar/Disney animation style, colorful, friendly.
        """

        with st.spinner("🎨 A IA está criando o mapa... (15s)"):
...
def consultar_gpt_pedagogico(api_key, dados, contexto_pdf="", regenerar=False):
    ...
    try:
        ...
        SUA MISSÃO: 
        1. Gerar o PEI TÉCNICO (Professores).
        2. Gerar o MAPA DO ALUNO (Gamificado).
        
        IMPORTANTE: MANTENHA AS ÁREAS TOTALMENTE SEPARADAS PELAS TAGS.
        NÃO COLOQUE DADOS TÉCNICOS (CIDs, REMÉDIOS) NA ÁREA DO ALUNO.
        ...
        [MAPA_ALUNO]
        (Texto em 1ª Pessoa, Emojis, Títulos em Negrito).
        
        ⚡ **Meu Mapa de Poderes** ⚡
        
        🧠 **Super Foco (Aprendizado)**
        [Como usar o hiperfoco]
...
# ==============================================================================
# 7. GERADOR PDF (VISUAL E HÍBRIDO)
# ==============================================================================
...
class PDF_Visual_Map(FPDF):
    def header(self):
        # Header Dourado (Paisagem)
        self.set_fill_color(255, 223, 0)
    ...
    def add_image_content(self, img_url):
        ...
        if img_url:
            try:
                ...
                self.set_xy(10, 35)
                self.cell(0, 10, "[Imagem não pode ser carregada]", 0, 1, 'C')
...
def gerar_pdf_final(dados, tem_anexo):
    ...
    if dados['ia_sugestao']:
        pdf.section_title("2. ESTRATÉGIAS TÉCNICAS")
        # Pega só o conteúdo da tag PEI_TECNICO
        raw_text = extrair_tag_ia(dados['ia_sugestao'], "PEI_TECNICO")
        if not raw_text: raw_text = dados['ia_sugestao'] # Fallback
        
        # Limpa o texto para o PDF (remove formatação markdown)
        clean_text = limpar_texto_pdf(raw_text)
        pdf.multi_cell(0, 6, clean_text)
        
    return pdf.output(dest='S').encode('latin-1', 'replace')

def gerar_pdf_visual_com_imagem(texto_mapa, img_url):
    pdf = PDF_Visual_Map(orientation='L', format='A4')
    pdf.add_page()
    
    # 1. Coloca a imagem no topo/centro
    pdf.add_image_content(img_url)
    
    # 2. Adiciona o texto gamificado embaixo ou ao lado
    pdf.set_y(140) # Posição abaixo da imagem
    pdf.set_font("Arial", '', 11)
    
    # Divide o texto em blocos para ficar bonito
...
with st.sidebar:
    ...
    st.markdown(f"<div style='font-size:0.75rem; color:#A0AEC0;'><b>PEI 360º v92.0 Visual Edition</b><br>Criado por<br><b>Rodrigo A. Queiroz</b><br>{data_atual}</div>", unsafe_allow_html=True)
...
# ... (ABAS 1 a 6 OMITIDAS - MANTENHA O ORIGINAL) ...
...
with tab8: # DOCUMENTO (TÉCNICO)
    render_progresso()
    st.markdown("### <i class='ri-file-pdf-line'></i> Documentação Oficial (Técnica)", unsafe_allow_html=True)
    if st.session_state.dados['ia_sugestao']:
...
with tab_mapa: # MEU MAPA (GAMIFICADO)
    render_progresso()
    st.markdown(f"""
    ...
    with col_text_map:
        ...
        if st.session_state.dados['ia_sugestao']:
            texto_mapa = extrair_tag_ia(st.session_state.dados['ia_sugestao'], "MAPA_ALUNO")
            if texto_mapa:
    ...
    # 2. IMAGEM DALL-E + PDF VISUAL
    with col_dalle_map:
        st.markdown("#### 🎨 Meu Quadro Visual (DALL-E)")
        
        texto_para_imagem = ""
        if st.session_state.dados['ia_sugestao']:
             texto_para_imagem = extrair_tag_ia(st.session_state.dados['ia_sugestao'], "MAPA_ALUNO")

        if st.button("✨ Criar/Atualizar Imagem do Mapa", type="primary", use_container_width=True):
            if texto_para_imagem and st.session_state.dados['hiperfoco']:
                url, err = gerar_imagem_dalle(api_key, st.session_state.dados, texto_para_imagem)
                if url:
        ...
        if st.session_state.dalle_image_url:
            ...
            # BOTÃO DE PDF VISUAL (COM A IMAGEM)
            pdf_visual = gerar_pdf_visual_com_imagem(texto_para_imagem, st.session_state.dalle_image_url)
            st.download_button(
                "📥 Baixar PDF Visual (Imagem + Texto)", 
                pdf_visual, 
                "Meu_Mapa_Visual.pdf", 
                "application/pdf", 
                type="primary",
