...
# Definição do Estado Completo (Com campos novos da V7.0)
default_state = {
...
# ==============================================================================
# 3. INTELIGÊNCIA ARTIFICIAL (FUNCIONALIDADES V7.0)
# ==============================================================================
@st.cache_data(ttl=3600)
def gerar_saudacao_ia(api_key):
    """Gera mensagem de boas-vindas na Home."""
    if not api_key: return "Bem-vindo ao PEI 360º. A inclusão transforma vidas."
    try:
        client = OpenAI(api_key=api_key)
        prompt = "Escreva uma frase curta (máx 20 palavras) e inspiradora para um professor sobre o impacto positivo do PEI. Tom acolhedor."
...
def gerar_noticia_ia(api_key):
    """Gera destaque de legislação ou neurociência."""
    if not api_key: return "Dica: Mantenha o PEI atualizado conforme a LBI."
    try:
        client = OpenAI(api_key=api_key)
        prompt = "Escreva um parágrafo curto (estilo 'Destaque') sobre uma novidade da legislação de inclusão (ex: Decreto 12.686) ou uma estratégia neurocientífica."
        res = client.chat.completions.create(model="gpt-4o-mini", messages=[{"role": "user", "content": prompt}], temperature=0.7)
        return res.choices[0].message.content
    except: return "A legislação atual reforça a obrigatoriedade do PEI."

def consultar_gpt_v7(api_key, dados, contexto_pdf=""):
    ...
    try:
        ...
        prompt_sistema = """
        Você é um Neuropsicopedagogo Sênior. Construa o PEI centrado no estudante.
        REGRAS DE FORMATAÇÃO:
        1. NÃO COLOQUE TÍTULO NO DOCUMENTO (O PDF já tem cabeçalho).
        2. Comece pelo título "1. PERFIL BIOPSICOSSOCIAL DO ESTUDANTE".
        3. Use CAIXA ALTA nos títulos numéricos (1., 2., 3., 4., 5., 6.).
        ...
        5. MONITORAMENTO E METAS (Novo: Indicadores de sucesso)
        6. PARECER FINAL
        """
        ...
        ESTRATÉGIAS ACESSO: {', '.join(dados['estrategias_acesso'])}{extra_acesso}
        ESTRATÉGIAS ENSINO: {', '.join(dados['estrategias_ensino'])}{extra_ensino}
...
# ==============================================================================
# 4. GERAÇÃO DE PDF E WORD (COM NOVOS CAMPOS)
# ==============================================================================
class PDF_V3(FPDF):
    def header(self):
...
def gerar_pdf_final(dados, tem_anexo):
    ...
    # 5. Monitoramento (Aparece se houver dados)
...
st.markdown("""
    ...
    .header-unified p { color: #004E92; margin: 0; font-size: 1.3rem; font-weight: 800; }

    /* ABAS PÍLULA (PILLS) */
    .stTabs [data-baseweb="tab-list"] { gap: 8px; padding-bottom: 10px; flex-wrap: wrap; }
    .stTabs [data-baseweb="tab"] {
        height: 40px; border-radius: 20px; padding: 0 25px; background-color: white;
    ...
    .rich-card:hover { transform: translateY(-5px); border-color: var(--brand-blue); box-shadow: 0 8px 15px rgba(0,78,146,0.1); }
    .rich-card h3 { margin: 10px 0 5px 0; font-size: 1.1rem; color: var(--brand-blue); font-weight: 700; }
    .rich-card p { font-size: 0.85rem; color: #718096; line-height: 1.3; }
    .rich-icon { font-size: 2.5rem; color: var(--brand-coral); margin-bottom: 10px; }

    /* CARD DE NOVIDADES IA */
    .highlight-card {
        background: linear-gradient(135deg, #fdfbfb 0%, #ebedee 100%); border-left: 5px solid #F6AD55;
        border-radius: 12px; padding: 20px; margin-top: 15px; margin-bottom: 20px;
        display: flex; align-items: center; gap: 15px; box-shadow: 0 4px 8px rgba(0,0,0,0.05);
    }
...
with st.sidebar:
    ...
    st.markdown(f"<div style='font-size:0.75rem; color:#A0AEC0;'><b>PEI 360º v7.0</b><br>Rodrigo A. Queiroz<br>{data_atual}</div>", unsafe_allow_html=True)
...
# --- ABAS (NOMES ORIGINAIS + MONITORAMENTO) ---
...
with tab4:
    ...
    with st.container(border=True):
        ...
        st.session_state.dados['potencias'] = c2.multiselect("Pontos Fortes", ["Memória Visual", "Lógica", "Criatividade", "Artes"], default=st.session_state.dados['potencias'])
    st.divider()
...
with tab5:
    ...
    c1, c2, c3 = st.columns(3)
    with c1:
        st.session_state.dados['estrategias_acesso'] = st.multiselect("Acesso", ["Tempo Estendido", "Apoio à Leitura"], default=st.session_state.dados['estrategias_acesso'])
...
# TAB 7: IA
with tab7:
    st.markdown("### <i class='ri-brain-line'></i> Consultoria IA", unsafe_allow_html=True)
    if st.button("GERAR PLANO AGORA", type="primary"):
        with st.spinner("Analisando..."):
            res, err = consultar_gpt_v7(api_key, st.session_state.dados, st.session_state.pdf_text)
            if res: st.session_state.dados['ia_sugestao'] = res; st.success("Sucesso!")
            else: st.error(err)
    if st.session_state.dados['ia_sugestao']: st.text_area("Texto", st.session_state.dados['ia_sugestao'], height=500)
        ...
        with c2:
            ...
            if uploaded_json:
                try:
                    ...
                    # Converte datas de string de volta para objeto
