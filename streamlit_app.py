...
import random
import requests
import tempfile # Adicionado para manipulação da imagem no PDF
...
# Função para extrair seções específicas para o PDF
...
def limpar_texto_pdf(texto):
    ...
    # Remove emojis básicos que quebram o FPDF
    t = t.replace('⚡', '').replace('🧠', '').replace('🌬️', '').replace('🕒', '').replace('📁', '').replace('🚶‍♂️', '').replace('🎨', '')
...
def gerar_imagem_dalle_inspiracional(api_key, dados_aluno):
    ...
    try:
        ...
        Theme: {hf}. 
        Subject: A young student character (approx age for {serie}) looking confident and heroic in a magical study world.
...
with st.sidebar:
    ...
    st.markdown(f"<div style='font-size:0.75rem; color:#A0AEC0;'><b>PEI 360º v96.0 Visual Final</b><br>Criado por<br><b>Rodrigo A. Queiroz</b><br>{data_atual}</div>", unsafe_allow_html=True)
