...
# ==============================================================================
# 6. INTELIGÊNCIA ARTIFICIAL (V95 - BLINDAGEM DE CONTEÚDO E IMAGEM INSPIRACIONAL)
# ==============================================================================
...
# --- FUNÇÃO DALL-E 3 (AGORA INSPIRACIONAL, SEM TENTAR FAZER TEXTO) ---
def gerar_imagem_dalle_inspiracional(api_key, dados_aluno):
    ...
    try:
        ...
        # PROMPT DE ARTE CONCEITUAL (INSPIRACIONAL)
        prompt_dalle = f"""
        A magical, inspiring concept art illustration in Pixar style.
        Theme: {hf}. 
        Subject: A young student hero (age appropriate for {serie}) standing confidently in a world made of {hf} elements.
        Atmosphere: Glowing, positive, organized, full of potential. 
        Visuals: Floating icons of books, calm clouds, and a shield, integrated into the environment.
...
def consultar_gpt_pedagogico(api_key, dados, contexto_pdf="", regenerar=False):
    ...
    try:
        ...
        # PROMPT ESTRUTURADO PARA SEPARAÇÃO TOTAL E BLINDAGEM
        prompt_sys = f"""
        ...
        2. Gerar o CONTEÚDO DO ALUNO (Gamificado e Simples).
        ...
        (Gere um guia visual em Markdown para o aluno. REGRAS RÍGIDAS: 
        1. NÃO inclua CID, diagnósticos, remédios ou problemas comportamentais aqui.
        2. Use linguagem de 'Superpoderes' e 'Missões'.
        3. Foco 100% positivo e prático).
...
def gerar_pdf_final(dados, tem_anexo):
    ...
    if dados['ia_sugestao']:
        ...
        # Extrai apenas o conteúdo da tag PEI_TECNICO
...
def gerar_pdf_visual_final(texto_mapa, img_url):
    # PDF em Paisagem ('L')
    pdf = PDF_Visual_Final(orientation='L', format='A4')
    pdf.add_page()
...
with st.sidebar:
    ...
    st.markdown(f"<div style='font-size:0.75rem; color:#A0AEC0;'><b>PEI 360º v95.0 Visual Final</b><br>Criado por<br><b>Rodrigo A. Queiroz</b><br>{data_atual}</div>", unsafe_allow_html=True)
...
with tab_mapa: # MEU MAPA (ROTEIRO + IMAGEM)
    ...
    with col_text_map:
        st.markdown("#### ⚡ Meus Poderes & Missões (Roteiro)")
        if st.session_state.dados['ia_sugestao']:
    ...
    with col_dalle_map:
        st.markdown("#### 🎨 Quadro Visual (Arte Inspiracional)")
        ...
        if st.button("✨ Criar Arte do Mapa", type="primary", use_container_width=True):
            if st.session_state.dados['hiperfoco']:
                # Chama a função INSPIRACIONAL (não tenta ler texto)
                url, err = gerar_imagem_dalle_inspiracional(api_key, st.session_state.dados)
                if url:
        ...
        if st.session_state.dalle_image_url:
            ...
            pdf_visual = gerar_pdf_visual_final(texto_para_imagem, st.session_state.dalle_image_url)
            st.download_button(
