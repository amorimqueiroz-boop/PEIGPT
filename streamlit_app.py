...
import glob
import random
import graphviz # BIBLIOTECA NATIVA DO STREAMLIT PARA MAPAS MENTAIS
...
default_state = {
    ...
    'ia_mapa_estudante': '' # NOVO CAMPO PARA O MAPA MENTAL
}
...
# --- PARSER DO MAPA MENTAL ---
def gerar_grafico_estudante(texto_mapa):
    # O texto virá no formato: [NÓ_PAI] -> [NÓ_FILHO]
    # Vamos criar um objeto Graphviz
    graph = graphviz.Digraph()
    graph.attr(rankdir='LR', bgcolor='transparent') # Esquerda para Direita
    graph.attr('node', shape='box', style='filled', color='#0F52BA', fontcolor='white', fontname='Nunito')
    ...
    for linha in linhas:
        if "->" in linha:
            ...
            # Estilização básica baseada em palavras-chave
            cor = "#0F52BA" # Azul Padrão
            if "Superpoder" in origem or "Hiperfoco" in origem: cor = "#D69E2E" # Dourado
            elif "Escola" in origem or "Aula" in origem: cor = "#3182CE" # Azul Claro
            elif "Casa" in origem or "Estudo" in origem: cor = "#38A169" # Verde
            elif "Ansied" in origem or "Ajuda" in origem: cor = "#E53E3E" # Vermelho
            
            graph.node(origem, label=origem, color=cor, style='filled, rounded')
            graph.node(destino, label=destino, color='white', fontcolor='#2D3748', style='filled, rounded', penwidth='2')
            graph.edge(origem, destino, color="#CBD5E0")
...
# ==============================================================================
# 5. ESTILO VISUAL (HOME VIBRANTE V68 + DASHBOARD V74)
# ==============================================================================
...
# ==============================================================================
# 6. INTELIGÊNCIA ARTIFICIAL (AGÊNCIA DO ESTUDANTE INCLUÍDA)
# ==============================================================================
...
def consultar_gpt_pedagogico(api_key, dados, contexto_pdf=""):
    ...
    try:
        ...
        SUA MISSÃO: Criar um PEI Técnico (para o professor) e um MAPA DE AÇÃO (para o aluno).
        ...
        [MAPA_ESTUDANTE]
        Crie uma estrutura visual de grafo (setas) com linguagem para CRIANÇA/ADOLESCENTE.
        Use: NÓ_PAI -> NÓ_FILHO.
        Exemplo:
        Superpoder -> Usar Minecraft na Matemática
        Escola -> Pedir tempo se cansar
        Casa -> Fazer pausas de 5 min
...
# ==============================================================================
# (Código do PDF igual ao anterior, omitido para brevidade, mas está incluso)
...
with tab_mapa:
    ...
    <div style="background: linear-gradient(90deg, #6B46C1 0%, #D69E2E 100%); padding: 25px; border-radius: 20px; color: white; margin-bottom: 20px;">
        <h3 style="margin:0; color:white;">🗺️ Mapa da Minha Jornada</h3>
        <p style="margin:5px 0 0 0;">Este espaço é seu! Veja como usar seus superpoderes para aprender melhor.</p>
    </div>
    """, unsafe_allow_html=True)
    
    if st.session_state.dados['ia_sugestao']:
        # Extrai o bloco do mapa
